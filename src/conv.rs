...
pub const fn extend(a: SoftF32) -> SoftF64 {
    let src_zero = 0;
    let src_one = 1;
    let src_bits = SoftF32::BITS;
    let src_sign_bits = SoftF32::SIGNIFICAND_BITS;
    let src_exp_bias = SoftF32::EXPONENT_BIAS;
    let src_min_normal = SoftF32::IMPLICIT_BIT;
    let src_infinity = SoftF32::EXPONENT_MASK;
    let src_sign_mask = SoftF32::SIGN_MASK;
    let src_abs_mask = src_sign_mask - src_one;
    let src_qnan = SoftF32::SIGNIFICAND_MASK;
    let src_nan_code = src_qnan - src_one;

    let dst_bits = SoftF64::BITS;
    let dst_sign_bits = SoftF64::SIGNIFICAND_BITS;
    let dst_inf_exp = SoftF64::EXPONENT_MAX;
    let dst_exp_bias = SoftF64::EXPONENT_BIAS;
    let dst_min_normal = SoftF64::IMPLICIT_BIT;
    ...
    SoftF64::from_repr(abs_result | (sign_result.wrapping_shl(dst_bits - src_bits)))
}

// Source: https://github.com/rust-lang/compiler-builtins/blob/3dea633a80d32da75e923a940d16ce98cce74822/src/float/trunc.rs#L4
pub const fn trunc(a: SoftF64) -> SoftF32 {
    let src_zero = 0_u64;
    let src_one = 1_u64;
    let src_bits = SoftF64::BITS;
    let src_exp_bias = SoftF64::EXPONENT_BIAS;

    let src_min_normal = SoftF64::IMPLICIT_BIT;
    let src_significand_mask = SoftF64::SIGNIFICAND_MASK;
    let src_infinity = SoftF64::EXPONENT_MASK;
    let src_sign_mask = SoftF64::SIGN_MASK;
    let src_abs_mask = src_sign_mask - src_one;
    let round_mask = (src_one << (SoftF64::SIGNIFICAND_BITS - SoftF32::SIGNIFICAND_BITS)) - src_one;
    let halfway = src_one << (SoftF64::SIGNIFICAND_BITS - SoftF32::SIGNIFICAND_BITS - 1);
    let src_qnan = src_one << (SoftF64::SIGNIFICAND_BITS - 1);
    let src_nan_code = src_qnan - src_one;

    let dst_zero = 0_u32;
    let dst_one = 1_u32;
    let dst_bits = SoftF32::BITS;
    let dst_inf_exp = SoftF32::EXPONENT_MAX;
    let dst_exp_bias = SoftF32::EXPONENT_BIAS;
    ...
    let underflow: u64 = underflow_exponent << SoftF64::SIGNIFICAND_BITS;
    let overflow: u64 = overflow_exponent << SoftF64::SIGNIFICAND_BITS;

    let dst_qnan = 1_u32 << (SoftF32::SIGNIFICAND_BITS - 1);
    let dst_nan_code = dst_qnan - dst_one;

    let sign_bits_delta = SoftF64::SIGNIFICAND_BITS - SoftF32::SIGNIFICAND_BITS;
    ...
    if a_abs.wrapping_sub(underflow) < a_abs.wrapping_sub(overflow) {
        ...
        let tmp = src_exp_bias.wrapping_sub(dst_exp_bias) << SoftF32::SIGNIFICAND_BITS;
        ...
        abs_result = (dst_inf_exp << SoftF32::SIGNIFICAND_BITS) as u32;
        abs_result |= dst_qnan;
        abs_result |= dst_nan_code
            & ((a_abs & src_nan_code) >> (SoftF64::SIGNIFICAND_BITS - SoftF32::SIGNIFICAND_BITS))
                as u32;
        ...
        if shift > SoftF64::SIGNIFICAND_BITS {
            ...
            abs_result = (denormalized_significand
                >> (SoftF64::SIGNIFICAND_BITS - SoftF32::SIGNIFICAND_BITS))
                as u32;
    ...
    SoftF32::from_repr(abs_result | sign.wrapping_shr(src_bits - dst_bits) as u32)
}
